...
{
    public partial class ChartTemperatureGtk : UserControl
    {
        public ChartTemperatureGtk()
        {
            InitializeComponent();
            InitializeChart();
        }
        ...
        private void InitializeChart()
        {
            ...
            chartControl1.PrimaryYAxis.Title = "ГТК";
        ...
        public virtual void UpdateChart(int year, string plot, List<WeatherDay> days, WeatherStyle weatherStyle)
        {
            ...
            chartControl1.Title.Text = $"ГТК на {plot} ГСУ (май - сентябрь) {year} годf";
            ...
            double[] arrayGtk = null;
            double[] arrayAverageGtk = null;
            switch (weatherStyle)
            {
                case WeatherStyle.ByDays:
                    break;
                case WeatherStyle.ByDecades:
                    arrayGtk = Gtk.CreateArrayGtkByDecades(year, days);
                    arrayAverageGtk = Gtk.CalculateAverageGtkByDecades(year, days);
                    break;
                case WeatherStyle.ByMonths:
                    arrayGtk = Gtk.CreateArrayGtkByMonth(year, days);
                    arrayAverageGtk = Gtk.CalculateAverageGtkByMonths(year, days);
                    break;
                default:
                    throw new ArgumentOutOfRangeException(nameof(weatherStyle), weatherStyle, null);
            }
            


            CreateSeries("ГТК", chartControl1, arrayGtk, ChartSeriesType.Column);
            CreateSeries("Средняя многолетняя ГТК", chartControl1, arrayAverageGtk, ChartSeriesType.Column);
        ...
        private void CreateSeries(string name, ChartControl chartControl, double[] values, ChartSeriesType type)
        {
            ...
            for (int i = start; i < end; i++)
            {
                ...
                {
                    series1.Points.Add(title, values[i]);
                }
                else
                {
