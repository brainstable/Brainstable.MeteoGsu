...
namespace Brainstable.Meteo
{
    public class FactoryMonths
    {
        ...
        public List<WeatherMonth> Create(List<WeatherDecade> decades)
        {
            foreach (WeatherDecade decade in decades)
            {
                string id = $"{decade.Year}{decade.Month.ToString("D2")}";
            ...
            {
                WeatherMonth p = new WeatherMonth();
