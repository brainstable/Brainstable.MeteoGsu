...
namespace Brainstable.Meteo
{
    public class FileMeteo2
    {
        ...
        public List<MeteoStation> GetMeteoStations()
        {
            ...
            string[] files = Directory.GetFiles(DIR_REPOS, "*.meta");
            int n = 0;
            for (int i = 0; i < files.Length; i++)
            {
                string[] arr = File.ReadAllLines(files[i]);
                MeteoStation st = new MeteoStation();

                st.Id = arr[1].Split(':')[1].Trim();
                st.Name = arr[2].Split(':')[1].Trim();
                st.Country = arr[3].Split(':')[1].Trim();
                //st.Link = arr[4];
                list.Add(st);
                n++;
            }
